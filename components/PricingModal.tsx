...
const PricingModal: React.FC<PricingModalProps> = ({ isOpen, onClose, onUpgrade }) => {
  ...
  const handleSubscribe = (tier: 'creator' | 'director') => {
    // ... (keep existing handleSubscribe)
    const url = STRIPE_LINKS[billingCycle][tier];
    if (url) window.open(url, '_blank');
    else alert("Payment link not configured for this selection.");
  };

  const handleTopUp = async (pack: typeof CREDIT_PACKS[0]) => {
    if (confirm(`Purchase ${pack.credits} credits for $${pack.price}?`)) {
      setIsProcessing(true);
      // Simulate API call
      await new Promise(r => setTimeout(r, 1500));
      await buyCredits(pack.credits, pack.price);
